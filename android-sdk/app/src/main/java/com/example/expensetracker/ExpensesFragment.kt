...
class ExpensesFragment : Fragment() {
    ...
    private val adapter = ExpenseAdapter(emptyList())
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        swipeRefresh.setOnRefreshListener { loadExpenses() }
    ...
    private fun loadExpenses() {
        ...
        Executors.newSingleThreadExecutor().execute {
            try {
                ...
                requireActivity().runOnUiThread {
                    swipeRefresh.isRefreshing = false
                    Toast.makeText(requireContext(), "Could not load expenses", Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
    ...
    private fun showAddExpenseDialog() {
        val cookie = SessionManager.getCookie(requireContext()) ?: return
        AddExpenseDialog(requireContext(), cookie) { loadExpenses() }.show()
    }
}
